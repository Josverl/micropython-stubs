...
def test_features(lcd, orient=lcd160cr.PORTRAIT):
    ...
    for i in range(300):
        ...
        fbuf.line(w // 2, h // 2, w // 2 + int(40 * math.cos(0.2 * i)), h // 2 + int(40 * math.sin(0.2 * i)), lcd.rgb(128, 255, 64))
        ...
        for phase in (-0.2, 0, 0.2):
            ...
            fbuf.blit(mlogo, x, y)
        for j in range(-3, 3):
            fbuf.text("MicroPython", 5, h // 2 + 9 * j + int(20 * math.sin(0.1 * (i + j))), lcd.rgb(128 + 10 * j, 0, 128 - 10 * j))
