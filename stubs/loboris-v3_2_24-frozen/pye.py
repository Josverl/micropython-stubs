...
class Editor:
    ...
    def display_window(self):
        ...
        self.hilite(1)
        self.wr(
            "{}{} Row: {}/{} Col: {}  {}".format(self.changed, self.fname, self.cur_line + 1, self.total_lines, self.col + 1, self.message)[
                : self.width - 1
            ]
        )
        self.clear_to_eol()
    ...
    def undo_add(self, lnum, text, key, span=1):
        self.changed = "*"
        if self.undo_limit > 0 and (len(self.undo) == 0 or key == 0 or self.undo[-1][3] != key or self.undo[-1][0] != lnum):
                        ...
                        if ni >= 0:
                            ...
                            if q == "q" or key == 0x11:
                                ...
                                self.content[self.cur_line] = (
                                    self.content[self.cur_line][: self.col] + rpat + self.content[self.cur_line][self.col + ni :]
                                )
                                self.col += len(rpat)
                                count += 1
                            else:
