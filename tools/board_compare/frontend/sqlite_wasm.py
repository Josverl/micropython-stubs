...
class SQLite:
    ...
    async def open_database(self, file_path: str) -> SQLDatabase:
        ...
        try:
            window.console.log(f"{_timestamp()} reading file_data from '{file_path}'...")
            ...
            window.console.log(f"{_timestamp()} creating db_array from file_data...")
            db_array = js.Uint8Array.new(file_data)
            window.console.log(f"{_timestamp()} creating Database instance from db_array...")
            # return self._sql["Database"].new(db_array)
            db = self._sql["Database"].new(db_array)
            window.console.log(f"{_timestamp()} Done creating Database.")
            return db
