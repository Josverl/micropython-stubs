...
# Global state
app_state = {"boards": [], "current_board": None, "db": None, "SQL": None, "board_utils_loaded": False}
...
async def load_database():
    ...
    try:
        ...
        SQL = await window.initSqlJs(
            ffi.to_js({"locateFile": lambda file, *args: f"https://cdnjs.cloudflare.com/ajax/libs/sql.js/1.13.0/{file}"})
        )
        app_state["SQL"] = SQL
...
def get_class_methods(module_id, class_id, board_context):
    ...
    try:
        stmt = app_state["db"].prepare("""
            ...
            ORDER BY um.name
        """)
        stmt.bind([module_id, class_id, board_context["version"], board_context["port"], board_context["board"]])
...
def get_module_functions(module_id, board_context):
    ...
    try:
        stmt = app_state["db"].prepare("""
            ...
            ORDER BY um.name
        """)
        stmt.bind([module_id, board_context["version"], board_context["port"], board_context["board"]])
