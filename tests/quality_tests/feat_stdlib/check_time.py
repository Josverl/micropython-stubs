...
# -----------------------
# Guard against direct tick arethmatic overflow,
# which is a common source of bugs in MicroPython code.

from time import ticks_ms

start = ticks_ms()
# will fail if the type: ignore is not required
while ticks_ms() - start < 100:  # type: ignore # stubs-ignore: version<1.28.0
    pass
    # if uart.any():  return uart.read().decode()
# will fail if the type: ignore is not required
print("timeout", ticks_ms() - start)  # type: ignore # stubs-ignore: version<1.28.0
...
while 1:
    now = ticks_ms()
    # will fail if the type: ignore is not required
    d = now - was  # type: ignore # stubs-ignore: version<1.28.0
    cycle += 1
    if d > 100:
        if gotcha == 0:
