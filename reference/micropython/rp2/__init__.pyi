...
# NOTE: ``_PIO_ASM_Program`` is the OPAQUE assembled-program type returned by
# ``@asm_pio`` and consumed by ``StateMachine``/``PIO``. It is defined as a
# (forward-referenced) class at the bottom of this module. Do **not** alias it
# to ``Callable`` — that masks attribute errors on the chainable instruction
# expressions inside the decorator body. Those chainable expressions are typed
# via ``rp2.asm_pio._PIOInstr`` instead.
